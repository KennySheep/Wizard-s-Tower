...
void DotSceneLoader::parseDotScene(btDynamicsWorld *phyWorld, const Ogre::String &SceneName, const Ogre::String &groupName, Ogre::SceneManager *yourSceneMgr, Ogre::SceneNode *pAttachNode, const Ogre::String &sPrependNode)
{
	m_phyWorld = phyWorld;
...
void DotSceneLoader::processTerrain(rapidxml::xml_node<>* XMLNode)
{
	...
	Ogre::Terrain *myTerrain = mTerrainGroup->getTerrain(0, 0);

	// Convert height data in a format suitable for the physics engine
	int size = myTerrain->getSize();
	float *data = myTerrain->getHeightData();
	Ogre::Real minHeight = myTerrain->getMinHeight();
	Ogre::Real maxHeight = myTerrain->getMaxHeight();
	Ogre::Vector3 position = myTerrain->getPosition();
	Ogre::Real scale = myTerrain->getWorldSize() / (myTerrain->getSize() - 1);

	Ogre::LogManager::getSingleton().logMessage(Ogre::String("[World::Terrain]")
	+ "; minHeight=" + Ogre::StringConverter::toString(myTerrain->getMinHeight())
	+ "; maxHeight=" + Ogre::StringConverter::toString(myTerrain->getMaxHeight())
	+ "; worldSize=" + Ogre::StringConverter::toString(myTerrain->getWorldSize())
	+ "; size=" + Ogre::StringConverter::toString(myTerrain->getSize())
	+  "; scale=" + Ogre::StringConverter::toString(scale)
	+ "; position=" + Ogre::StringConverter::toString(myTerrain->getPosition()));
	...
	btHeightfieldTerrainShape *mTerrainShape = new btHeightfieldTerrainShape( size, size, mTerrainHeights, 1 /*ignore*/, minHeight, maxHeight, 1, PHY_FLOAT, true);
	mTerrainShape->setUseDiamondSubdivision(true);
	mTerrainShape->setLocalScaling(localScaling);

	btRigidBody* mGroundBody = new btRigidBody(0, new btDefaultMotionState(), mTerrainShape);
	mGroundBody->setFriction(0.8f);
	mGroundBody->setHitFraction(0.8f);
	mGroundBody->setRestitution(0.6f);

	mGroundBody -> getWorldTransform().setOrigin(BtOgre::Convert::toBullet(position) + btVector3(0, 30, 0)); // adjust this number to suit your terrain height//////////////////////////////////////////////////this! change this!
	...
	m_phyWorld->addRigidBody(mGroundBody);
...
void DotSceneLoader::processEntity(rapidxml::xml_node<>* XMLNode, Ogre::SceneNode *pParent)
{
	...
	btVector3 bulletPosition = BtOgre::Convert::toBullet(position);

	btDefaultMotionState* groundState = new btDefaultMotionState(
		btTransform(btQuaternion(0,0,0,1),bulletPosition));

	BtOgre::StaticMeshToShapeConverter converter2(pEntity);
